...
def card_tracker(cards_name, target, quality):
    url = 'https://api.x.immutable.com/v1/orders?direction=asc&include_fees=true&order_by=buy_quantity&page_size=1&sell_token_address=0xacb3c6a43d15b907e8433077b6d38ae40936fe2c&sell_token_type=ERC721&status=active&sell_token_name="%s"&sell_metadata={"quality":["%s"]}&buy_token_type=ETH'%(cards_name, quality)
    ...
    if price <= target:
        print("Il faut acheter la carte", cards_name)
        if notif == 'Oui':
            title = "Il faut acheter la carte" + cards_name
            body = "Le prix de la carte" + cards_name + "est de" + price + "eth et la target est de" + target
...
while True:
    ...
    quality = input("Qualité de la carte ? (Meteorite,Shadow,Gold,Diamond) :  ")
    target = float(input("Seuil de vente  ? :  "))
    ...
    if next == "Non":
        ...
        if save == "Oui":
            with open("list_cards.txt", "wb") as fp:  # save
                ...
                print("sauvegarde")
        notif = input("Veux-tu les notifications ? (Oui, Non) :")
        break
